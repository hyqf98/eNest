...
| API | `window.enest`（zapi 为 deprecated 别名），IPC 权限门禁 |
...
| [enest API](api.md) | 完整方法参考：UI / 主题 / 设置 / 存储 / 剪贴板 / 通知 / 生命周期 |
...
?> 建议顺序：**快速开始 → plugin.json → enest API → 生命周期 → 示例**。示例见本页与仓库 `plugins-samples/hello`。
